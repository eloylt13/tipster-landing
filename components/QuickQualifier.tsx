...
export default function QuickQualifier({ content, answers, onAnswer }: QuickQualifierProps) {
  ...
  return (
    <section className="pt-2 sm:pt-4">
      <div className="overflow-hidden rounded-[2.25rem] border border-white/10 bg-[linear-gradient(180deg,_rgba(255,255,255,0.07),_rgba(255,255,255,0.035))] shadow-[0_28px_90px_rgba(0,0,0,0.32)] backdrop-blur-sm">
        <div className="grid gap-6 px-4 py-5 sm:px-6 sm:py-6 lg:grid-cols-[1.08fr_0.92fr] lg:gap-8 lg:px-7 lg:py-7">
          <div className="space-y-4 lg:pt-1">
            ...
            <div className="space-y-3">
              ...
              <p className="max-w-2xl text-sm leading-6 text-slate-300 sm:text-[0.98rem] sm:leading-7">
                {content.description}
              </p>
            </div>
          </div>
                      ...
                      <button
                        ...
                        className={`group min-h-14 rounded-[1.15rem] border px-4 py-3.5 text-left text-[0.95rem] leading-6 transition duration-200 ${
                          selected
                            ...
                            : "border-white/10 bg-white/[0.04] text-slate-100 hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/[0.075] hover:text-white"
                        }`}
                      >
                ...
                <div className="rounded-[1.35rem] border border-white/10 bg-white/[0.04] p-4 sm:p-5">
                  ...
                  <div className="mt-4 grid gap-2.5 sm:grid-cols-3">
                    {summaryItems.map((item) => (
                      <div
                        key={item.label}
                        className="rounded-[1rem] border border-white/10 bg-black/20 px-3.5 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]"
                      >
                        <p className="text-[0.66rem] font-semibold uppercase tracking-[0.24em] text-slate-400">
                          {item.label}
                        </p>
